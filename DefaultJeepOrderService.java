...
@Override
  public Order createOrder(OrderRequest orderRequest) {
    ...
    Tire trim = getTire(orderRequest);
    List<Option> options = getOption(orderRequest);
    BigDecimal price = jeep.getBasePrice().add(color.getPrice().add(engine.getPrice().add(trim.getPrice())));
return jeepOrderDao.saveOrder(customer, jeep, color, engine, trim, price, options);}
  /**
   * 
   * @param orderRequest
   * @return
